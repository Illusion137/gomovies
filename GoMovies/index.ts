...
import { GoMovie_Display_Content, GoMovieTab, GoMovieSearchPage } from './GoMovie';
import { Country_gm, Genre_gm, Path_gm, MonoPath_gm } from './enumsGM';

const GOMOVIES_BASE_URL = "https://gomovies.ms/";
const GOMOVIES_BASE_POST_URL = "https://gomovies.ms/ajax/";
...
const GOMOVIES_DEFAULT_POST_HEADERS = {
    "accept": "*/*",
    "accept-language": "en-US,en;q=0.9",
...
function gomovies_flw_item_to_gomovie_display_content(content_html : Element) : GoMovie_Display_Content{
    ...
    return {
        ...
        "href":             content_html.querySelector("a")?.href, 
...
function gomovies_page_html_to_search_page_data(page_html : string, page = 0) : GoMovieSearchPage {
    const { window } = new JSDOM(page_html);
    const contents = window.document.querySelectorAll('.flw-item, section');
...
export async function gomovies_search_autocomplete(search_query : string) : Promise<GoMovie_Display_Content[]> {
    ...
    const search_autocomplete_ajax_html_data = (await axios.post("search", `keyword=${search_query.replaceAll(' ', '+')}`, {"baseURL": GOMOVIES_BASE_POST_URL,"headers": GOMOVIES_DEFAULT_POST_HEADERS,}) ).data;
...
export async function gomovies_search_mono_path(mono_path : MonoPath_gm, page = 0) : Promise< GoMovieSearchPage > {
    return await gomovies_get_page(mono_path, page);
}
export async function gomovies_search_by_query(search_query : string, page = 0) : Promise< GoMovieSearchPage > {
    search_query = search_query.trim().replaceAll(/[^a-zA-Z0-9 ,]/g,'').trim();
    if(search_query == '') return null;
...
export async function gomovies_movie(){
    
}
export async function gomovies_watch_movie(movie_href : string){
    const watch_movie_html_data = (await axios.get("watch-movie/watch-five-nights-at-freddys-gomovies-41631.10042261", {"baseURL": GOMOVIES_BASE_URL, "headers": GOMOVIES_DEFAULT_HEADERS})).data;
    console.log(watch_movie_html_data);
}
export async function gomovies_tv_show(){}
export async function gomovies_watch_tv_show(){}
export async function gomovies_get_source(){}
